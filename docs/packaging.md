...
建议示例目录：`src/python_learning_lab/advanced/imports_lab/`
...
```python
from python_learning_lab.errors import parse_port
...
```toml
[project.scripts]
python-learning-lab = "python_learning_lab.cli:main"
```
...
def compatibility(session, constraint):
    ...
    session.install("-e", ".[dev]")
    session.run("pytest", "tests/test_pydantic_lab.py")
```
...
build
→ 构建 sdist/wheel 并运行 twine check
...
4. 新增 `build` 与 `package_smoke`，确保测试的是真实 wheel。
5. 新增 `release_check`，失败时不允许继续到发布步骤。
6. 比较 Nox session 与手写 shell 命令在跨平台、隔离和可读性上的差异。
...
用 Nox 编排多版本测试、Ruff、类型检查、构建、twine 检查和 wheel smoke test；
再用 uv 作为 session 后端运行同一套流程。
...
6. 新增 Nox sessions，统一测试、检查、构建和 wheel smoke test。
