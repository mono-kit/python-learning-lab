...
- 同一章切换到下一题时，先明确宣布上一题结束，再展示下一题的函数契约、输入输出和
  实际目标；用户理解问题背景后才介绍所需的新 API 或工具，不能无过渡地直接抛出下一题
  的实现手段。
- 用户掌握基础模型后，提高单次教学密度，把两到三个紧密相关的概念组成一个完整场景，
  减少只检查单个名词或机械语法的低难度问答。
- 练习应具有适度的综合性和真实工程背景，但题目中出现的语法、框架 API、领域类型和
  测试工具必须已经在本节或前置复习中讲解；不能用未教学内容制造难度。
